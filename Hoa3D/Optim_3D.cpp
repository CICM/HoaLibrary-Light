...
namespace Hoa3D
{
    ...
    void Optim::setMode(Mode mode)
    {
        ...
        else
        {
            ...
            {
                m_harmonics[i] =
                (long double)((long double)factorial(m_order + 1.) *
                              (long double)factorial(m_order)) /
                (long double)(
                              (long double)factorial(m_order + 1. + getHarmonicDegree(i)) *
                              (long double)factorial(m_order - getHarmonicDegree(i))
                              ) *
                gain;
            }
        }
